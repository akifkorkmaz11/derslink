// YKS Admin Panel JavaScript
console.log('YKS Admin.js yuklendi');
...
class AdminService {
    ...
    // Tüm kullaniciları getir (program bazlı filtreleme ile)
    ...
    // Ogretmen programını getir (program bazlı filtreleme ile)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (currentPage.includes('admin-yks.html')) {
            // YKS admin paneli
            await loadYKSAdminData();
        } else if (currentPage.includes('admin-lgs.html')) {
            // LGS admin paneli
...
// YKS admin paneli verilerini yükle
async function loadYKSAdminData() {
    console.log('YKS admin paneli verileri yükleniyor...');
...
async function loadStatistics(adminService) {
    try {
        ...
        if (classesResult.success && usersResult.success && teachersResult.success) {
            ...
            // YKS öğretmen sayısını hesapla (TYT veya AYT öğretmenleri)
            const totalTeachers = teachersResult.teachers.filter(teacher => 
                teacher.status === 'active' && 
                teacher.specialties && 
                (teacher.specialties.toLowerCase().includes('tyt') || 
                 teacher.specialties.toLowerCase().includes('ayt'))
...
async function loadTeacherSchedules(adminService) {
            try {
                console.log('YKS Öğretmen programları yükleniyor...');
                ...
                if (schedulesResult.success && schedulesResult.schedules) {
                    ...
                    // YKS programlarını göster, NULL programları da dahil et (eski veriler için)
                    schedulesResult.schedules = schedulesResult.schedules.filter(schedule => 
                        schedule.program === 'YKS' || !schedule.program
                    );
                    
                    console.log(`YKS programları filtrelendi: ${originalCount} → ${schedulesResult.schedules.length}`);
                }
        ...
        if (!teacherScheduleGrid) {
            ...
            return;
                        ...
                        // Aktif öğretmenleri filtrele (YKS öğretmenleri)
                ...
                console.log('YKS öğretmenleri:', activeTeachers.length);
                console.log('YKS öğretmen listesi:', activeTeachers.map(t => t.name));
        ...
        console.log('YKS Öğretmen programları yüklendi');
    } catch (error) {
...
async function loadTeacherSchedule() {
    try {
        ...
        // Aktif YKS öğretmenlerini filtrele (TYT veya AYT öğretmenleri)
        let activeTeachers = teachersResult.teachers.filter(teacher => 
            teacher.status === 'active' && 
            teacher.specialties && 
            (teacher.specialties.toLowerCase().includes('tyt') || 
             teacher.specialties.toLowerCase().includes('ayt'))
...
async function openScheduleEditModal(classData) {
    ...
    const adminService = new AdminService();
    const teachersResult = await adminService.getAllTeachers();
    const allActiveTeachers = teachersResult.success ? teachersResult.teachers.filter(t => t.status === 'active') : [];
    
    // YKS öğretmenlerini filtrele (TYT veya AYT öğretmenleri)
    const activeTeachers = allActiveTeachers.filter(teacher => 
        teacher.specialties && 
        (teacher.specialties.toLowerCase().includes('tyt') || 
         teacher.specialties.toLowerCase().includes('ayt'))
                ...
                <tr class="schedule-row" data-schedule-id="${schedule.id}">
                    ...
                    <td>
                        <select class="subject-select" onchange="updateScheduleField('${schedule.id}', 'subject', this.value)">
                            <!-- TYT Dersleri -->
                            <option value="TYT Matematik" ${schedule.subject === 'TYT Matematik' ? 'selected' : ''}>TYT Matematik</option>
                            <option value="AYT Matematik" ${schedule.subject === 'AYT Matematik' ? 'selected' : ''}>AYT Matematik</option>
                            <option value="Geometri" ${schedule.subject === 'Geometri' ? 'selected' : ''}>Geometri</option>
                            <option value="Fizik" ${schedule.subject === 'Fizik' ? 'selected' : ''}>Fizik</option>
                            <option value="Kimya" ${schedule.subject === 'Kimya' ? 'selected' : ''}>Kimya</option>
                            <option value="Biyoloji" ${schedule.subject === 'Biyoloji' ? 'selected' : ''}>Biyoloji</option>
                            <option value="Türk Dili ve Edebiyatı" ${schedule.subject === 'Türk Dili ve Edebiyatı' ? 'selected' : ''}>Türk Dili ve Edebiyatı</option>
                            <option value="Edebiyat" ${schedule.subject === 'Edebiyat' ? 'selected' : ''}>Edebiyat</option>
                            <option value="Coğrafya" ${schedule.subject === 'Coğrafya' ? 'selected' : ''}>Coğrafya</option>
                            <option value="Tarih" ${schedule.subject === 'Tarih' ? 'selected' : ''}>Tarih</option>
                            <option value="Felsefe" ${schedule.subject === 'Felsefe' ? 'selected' : ''}>Felsefe</option>
                            <option value="İngilizce" ${schedule.subject === 'İngilizce' ? 'selected' : ''}>İngilizce</option>
                            <option value="DKAB" ${schedule.subject === 'DKAB' ? 'selected' : ''}>DKAB</option>
                        </select>
                    </td>
                    <td>
...
async function addNewSchedule(classId) {
    ...
    const subject = prompt('Ders? (TYT Matematik, AYT Matematik, Geometri, Fizik, Kimya, Biyoloji, Türk Dili ve Edebiyatı, Edebiyat, Coğrafya, Tarih, Felsefe, İngilizce, DKAB)');
    if (!subject) return;
    
    // YKS öğretmenlerini filtrele (TYT veya AYT öğretmenleri)
    const yksTeachers = activeTeachers.filter(teacher => 
        teacher.specialties && 
        (teacher.specialties.toLowerCase().includes('tyt') || 
         teacher.specialties.toLowerCase().includes('ayt'))
    );
    ...
    // Öğretmen seçeneklerini oluştur
    const teacherOptions = yksTeachers.map(t => `${t.name} - ${t.branch}`).join(', ');
...
// Öğretmen seçim listesini güncelle (YKS öğretmenleri - TYT veya AYT)
function updateTeacherSelect(teachers) {
    ...
    if (teacherSelect) {
        ...
        // YKS öğretmenlerini filtrele ve ekle (TYT veya AYT)
...
async function updateMainPageTeachers() {
    try {
        ...
        if (result.success) {
            // Aktif YKS öğretmenlerini filtrele (TYT veya AYT öğretmenleri)
            const activeTeachers = result.teachers.filter(teacher => 
                teacher.status === 'active' && 
                teacher.specialties && 
                (teacher.specialties.toLowerCase().includes('tyt') || 
                 teacher.specialties.toLowerCase().includes('ayt'))
